...
if args.csv:
    ...
    x_den, z_den = model.x_dense, model.z_model
...
# --------------------------------------------------------------------
    ...
    # generate axes x
    x_den = np.arange(0.0, args.length + args.dx, args.dx)
    # Empirical value for "A"
    A_emp = dean_A_from_d50_no_calib(args.d50)
    z_den = dean1991(x_den, A_emp, args.sl)
...
# --------------------------------------------------------------------
x_min, x_max = 0.0, float(x_dean.max() if x_obs.size == 0 else max(x_obs.max(), x_den.max()))
y_bottom = max(np.max(z_dean), args.sl) + 0.1 if z_obs.size == 0 else max(z_obs.max(), z_den.max()) * 1.05
...
ax.fill_between(x_den, z_den, y_bottom, color=DARKSAND, alpha=0.7, zorder=3) #, label="Dean area")
...
ax.plot(x_den, z_den, "r--", lw=1.5, label="Dean (1991)", zorder=5)
