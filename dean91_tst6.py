...
class DeanTest:
    def __init__(self):
        ...
        # y_bottom = valor mais profundo (mais positivo para baixo)
        self.y_bottom = float(np.nanmax(self.yr))
        # y_top = topo (acima de HTL/observado)
        ...
        # (5) Dean (segmento) + fill escuro (Polygon reaproveitado)
        (self.dean_line,) = self.ax.plot([], [], "r--", lw=1.8, label="Dean Profile", zorder=5)
        self.sand_dean_poly = None  # Polygon do fill escuro (reaproveitado)
        ...
        # -------- slider (debounce) fora do gráfico --------
    ...
    def _really_update(self, x0_abs):
        ...
        # areia escura sob o Dean (Polygon estável, reaproveitado)
        verts = self._build_fill_verts(x_seg, y_seg, self.y_bottom)
        ...
        # métricas do segmento — atualiza SOMENTE o texto do item "Dean Profile" na legenda
